...
(function($) {
    ...
    GCube = function(element) {
        ...
        this.container.append(this.cube.domElement);//this.boundingBox);
    ...
    GCube.prototype = {
        ...
        setup: function() {
            ...
            twist = this.element.find('g-twist').text();
            ...
            jQuery.fn.extend({
                ...
                gscramble: function (val) {
                    self.gscramble(val);
                },
                ghighlight: function (val) {
                    self.ghighlight(val);
                },
                gtext: function (val) {
                    self.gtext(val);
                },
                gflorian: function (val) {
                    self.gflorian(val);
                },
                gtwist: function (val) {
                    self.gtwist(val);
        ...
        gtwist: function (command) {
            if (typeof command === 'string' && command.length > 0) {
                var cmd = algparse(command);
                this.cube.mouseControlsEnabled = false;

                var gcube = this; // Keep a reference to the current gCube.

                gcube.cube.twistDuration = parseInt(200);
                gcube.cube.twist(cmd);
            }
            return this;
        }
    };
    ...
    //var stylesheet = $('<link rel="stylesheet" type="text/css" href="lib/gcube.css" />');
